...
function initWebsocket() {
  return eventChannel((emitter: any) => {
    ...
    ws.onopen = (ev: Event) => {
      console.log('Opened @ticker WebSocket', ev);
      return emitter({ type: actionTypes.TICKER_WS_OPEN, ws })
    }

    ws.onerror = (ev: ErrorEvent) => {
      console.error('Error at @ticker WebSocket', ev);
      return emitter({ 
    ...
    ws.onmessage = (ev: MessageEvent) => {
      ...
      try {
        ...
        console.error('Error at parsing @ticker WebSocket data');
      }
    ...
    return () => {
      ws.close();
      console.log('Closed @ticker WebSocket');
    }
  })
}
