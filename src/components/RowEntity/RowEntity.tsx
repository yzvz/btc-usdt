...
function RowEntity(props: Entity) {
  ...
  return (
    <div className={styles.row} tabIndex={0}>
      ...
      {props.entity.amount ? <span className={[styles.cell, styles.amount].join(' ')}>{props.entity.amount}</span> : null}
      {props.entity.time ? <span className={[styles.cell, styles.time].join(' ')}>{timeFormatted}</span> : null}
      {props.entity.total ? <span className={[styles.cell, styles.total].join(' ')}>{props.entity.total}</span> : null}
    </div>
  );
}
